...
class FactScreenTest {
    ...
    fun testLoadingState() {
        ...
        composeTestRule.activity.setContent {
            EdisonAndroidExerciseTheme {
                FactScreen(viewModel = FakeLoadingFactViewModel(factService, dataStoreRepository))
            }
        }

        composeTestRule.onNodeWithTag(FactTestTag.FACT_BODY).assertDoesNotExist()
        composeTestRule.onNodeWithTag(FactTestTag.ERROR_CONTENT).assertDoesNotExist()
        composeTestRule.onNodeWithTag(FactTestTag.LOTTIE_ANIMATION).assertIsDisplayed()
    }
    ...
    fun testSuccessState() {
        ...
        composeTestRule.activity.setContent {
            EdisonAndroidExerciseTheme {
                FactScreen(viewModel = FakeSuccessFactViewModel(factService, dataStoreRepository))
            }
        }
    ...
    fun testErrorState() {
        ...
        composeTestRule.activity.setContent {
            EdisonAndroidExerciseTheme {
                FactScreen(viewModel = FakeErrorFactViewModel(factService, dataStoreRepository))
            }
        }

        composeTestRule.onNodeWithTag(FactTestTag.ERROR_CONTENT).assertIsDisplayed()
        composeTestRule.onNodeWithTag(FactTestTag.TRY_AGAIN_BUTTON).assertIsDisplayed()
    }
}